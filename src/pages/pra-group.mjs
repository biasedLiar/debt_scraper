  ...
  if (possibleElements.length !== 0) {
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      const accountReference = await account.evaluate(el => {
        const element = el.querySelector('#accountReferenceId strong');
        return element ? element.textContent.trim() : null;
      });
      ...
      // Extract amount (second .text--big strong element)
      await account.waitForSelector('.text--big strong', { visible: true });
      const amount = await account.evaluate(el => {
        const elements = el.querySelectorAll('.text--big strong');
        return elements.length > 1 ? elements[1].textContent.trim() : null;
      });
